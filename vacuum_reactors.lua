...
local function replace_depleted_cooling_cell(reactor, item, slot)
	...
	if slot_for_depleted_cooling_cell ~= nil then
		reactor.transposer.transferItem(
			reactor.transposer_sides.reactor_chamber,
			reactor.transposer_sides.depleted_cooling_cells_side,
			1,
			slot,
			slot_for_depleted_cooling_cell
		)
	end

	reactor.transposer.transferItem(
		reactor.transposer_sides.full_cooling_cells_side,
		reactor.transposer_sides.reactor_chamber,
		1,
		new_cooling_cell_slot,
		slot
	)

	log_info(
		"Replaced cooling cell in reactor " .. get_short_address(reactor.transposer) .. " in slot " .. tostring(slot)
	)
...
local function replace_depleted_fuel_rod(reactor, item, slot)
	...
	if slot_for_depleted_fuel_rod ~= nil then
		reactor.transposer.transferItem(
			reactor.transposer_sides.reactor_chamber,
			reactor.transposer_sides.general_provider,
			1,
			slot,
			slot_for_depleted_fuel_rod
		)
	end
...
local function get_lsc_proxy()
	local gt_machines =
		{ table.unpack(component.list("gt_machine", true)), table.unpack(component.list("gt_batterybuffer", true)) }
...
local function is_reactor_transposer(transposer_sides)
	return transposer_sides.reactor_chamber ~= nil
		and transposer_sides.temp_storage ~= nil
		and transposer_sides.general_provider ~= nil
		and transposer_sides.full_cooling_cells_side ~= nil
		and transposer_sides.depleted_cooling_cells_side ~= nil
end
...
local function identify_controlled_reactors(reactor_chambers, reactor_transposers, redstone_ios)
	...
	for _, redstone_io in ipairs(redstone_ios) do
		...
		for _, reactor_chamber in ipairs(reactor_chambers) do
			if reactor_chamber.producesEnergy() then
				if found then
					error(
						"Found multiple Reactor Chambers connected to a single Redstone IO ["
							.. redstone_io.address
							.. "]."
					)
				end
				...
				print(
					"Found Reactor Chamber ["
						.. reactor_chamber.address
						.. "] matching Redstone IO ["
						.. redstone_io.address
						.. "]."
				)
	...
	for _, reactor_transposer in ipairs(reactor_transposers) do
		...
		local reactor_plating_slot =
			find_reactor_plating_slot(reactor_transposer.proxy, reactor_transposer.sides.general_provider)
		if reactor_plating_slot == nil then
			error("Could not find reactor plating item in the general provider inventory.")
		end

		reactor_transposer.proxy.transferItem(
			reactor_transposer.sides.general_provider,
			reactor_transposer.sides.reactor_chamber,
			1,
			reactor_plating_slot,
			1
		)
		...
		for _, reactor in ipairs(reactors) do
			if reactor.reactor_chamber.getMaxHeat() > reactor.base_max_heat then
				if found then
					error(
						"Found multiple Reactor Chambers connected to a single Transposer ["
...
local function create_widgets(lsc, reactors)
	...
	local lsc_widget = {
		...
		draw = function(widget, tick)
			...
			local average_net_input_eut = lsc_status.avg_input_eut
				- lsc_status.avg_output_eut
			...
			gpu.set(
				...
				"LSC: "
					...
					.. "EU / "
					.. format_number(lsc_status.total_capacity_eu)
					.. "EU   ("
					.. tostring(lsc_fill_pct)
					.. "%)"
			)
			gpu.set(
				widget.min_x + 2,
			...
			gpu.set(
				...
				"I/O [EU/t]: +"
					...
					.. " = "
					.. format_number(average_net_input_eut)
					.. "EU/t"
					.. empty_or_full_message
			)
			...
			if lsc_status.needs_maintenance then
				...
				gpu.set(
					widget.min_x + 2 + string.len("LSC maintenance status: "),
					widget.min_y + 5,
					"Working perfectly"
				)
			end
...
local function main()
	...
	local function guarded_main()
		...
		local interrupted_event_id = event.listen("interrupted", function()
			keep_alive = false
		end)
